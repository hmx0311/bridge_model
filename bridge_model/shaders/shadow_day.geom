...
void main()
{
	...
	float NdotD = dot(normal, sun.light_dir_and_radius.xyz);
	float offset_scale = NdotD > 0 ? 0.1 * sqrt(1 / (NdotD * NdotD) - 1) + 0.01 : 0;
	for(int i = 0; i < CSM_LEVELS; i++)
	{
		float offset = -dot(sun.light_dir_and_radius.xyz, vec3(sun_shadow.view_proj[i][0].z, sun_shadow.view_proj[i][1].z, sun_shadow.view_proj[i][2].z)) * offset_scale;
		for(int j = 0; j < 3; j++)
		{
